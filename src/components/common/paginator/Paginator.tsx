...
export const Paginator: React.FC<PropsType> = ({
  ...
  const pages: Array<ReactNode> = Array(pagesCount)
    .fill('')
    .map((_, i) => {
      return (
        <span
          ...
          className={`${styles.paginationSpan} ${
            currentPage === i + 1 && styles.selectedPage
          }`}
        >
          {' '}
          {i + 1}{' '}
  ...
  return (
    <div className={styles.paginator}>
      {lastPage > paginatorPagesCount - 1 ? (
        <button onClick={prevPage}>PREVIOUS</button>
      ) : (
        ''
      )}
      {showLessSpans(lastPage)}
      {lastPage < pages.length - paginatorPagesCount ? (
